  ...
  return (
    <div className="border-b border-indigo-100">
      <button
        onClick={onClick}
        className={`
          ${isOpen ? 'bg-indigo-50' : isEven ? 'bg-white' : 'bg-gray-50'}
          w-full py-3 px-4 flex items-center justify-between
          text-left transition-colors hover:bg-indigo-100 rounded-lg
        `}
      >
        <h3 className="flex items-center gap-2 text-[16px] leading-[1.2] font-medium text-gray-900 pr-8">
          <BiHelpCircle className="text-indigo-600 w-4 h-4" />
          {question}
        </h3>
        <div
      ...
      >
        <div className="p-4 pt-2 text-gray-600">{answer}</div>
      </div>
    </div>
  );
};
